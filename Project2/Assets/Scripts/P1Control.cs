...
public class P1Control : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            movement = new Vector3(-Input.GetAxisRaw("Vertical"/*"JoyConVerticalP1"*/), 0, Input.GetAxisRaw("Horizontal"/*"JoyConHorizontalP1"*/));
            LookUpdate();
            KeyInputUpdate();
            LeftSlapUpdate();
            RightSlapUpdate();
    ...
    void KeyInputUpdate()
    {
        ...
        if (Input.GetKeyDown(KeyCode.H/*"JoyConLeftSlapP1"*/) && !isLeftReturn)
        {
            transform.GetChild(0).GetChild(0).gameObject.AddComponent<BoxCollider>();
            isLeftArmSlap = true;
        }
        if (Input.GetKeyDown(KeyCode.J/*"JoyConRightSlapP1"*/) && !isRightReturn)
        {
