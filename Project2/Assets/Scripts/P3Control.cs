...
public class P3Control : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            movement = new Vector3(-Input.GetAxisRaw("JoyConVerticalP3"), 0, Input.GetAxisRaw("JoyconHorizontalP3"));
            LookUpdate();
            KeyInputUpdate();
            LeftSlapUpdate();
            RightSlapUpdate();
    ...
    void KeyInputUpdate()
    {
        ...
        if (Input.GetKeyDown("JoyConLeftSlapP3") && !isLeftReturn)
        {
            transform.GetChild(0).GetChild(0).gameObject.AddComponent<BoxCollider>();
            isLeftArmSlap = true;
        }
        if (Input.GetKeyDown("JoyConRightSlapP3") && !isRightReturn)
        {
