...
public class P2Control : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            movement = new Vector3(-Input.GetAxisRaw("Vertical2"/*"JoyConVerticalP2"*/), 0, Input.GetAxisRaw("Horizontal2"/*"JoyConHorizontalP2"*/));
            LookUpdate();
            KeyInputUpdate();
            LeftSlapUpdate();
            RightSlapUpdate();
    ...
    void KeyInputUpdate()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Keypad5/*"JoyConLeftSlapP2"*/) && !isLeftReturn)
        {
            transform.GetChild(0).GetChild(0).gameObject.AddComponent<BoxCollider>();
            isLeftArmSlap = true;
        }
        if (Input.GetKeyDown(KeyCode.Keypad6/*"JoyConRightSlapP2"*/) && !isRightReturn)
        {
